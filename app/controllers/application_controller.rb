class ApplicationController < ActionController::Base
  ...
  before_filter :current_user, :fetch_static_pages, :require_first_last_name, :initial_broadcasts, :clear_broadcasts, :require_not_new_user
  ...
  def require_first_last_name
    if current_user && (current_user.first_name.blank? || current_user.last_name.blank?)
      ...
      redirect_to custom_user_path(current_user.custom_url, other: true)
    end
  end
  
  def require_not_new_user
    if current_user and current_user.new == true
  ...
  def push_broadcast(intent, options = {})
    if current_user
      ...
      when :friend_request
        message = "#{current_user.name} wants to be buddies."
      when :friend_accept
        message = "#{current_user.name} accepted your buddy request."
      when :shared_note
  ...
  def initial_broadcasts
    ...
    @current_broadcasts = @broadcasts.where(:current => true) if current_user
  end

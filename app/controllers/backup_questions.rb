...
def index
  ...
  def create
    ...
    #  render_questions
      ...
      @questions = Offering.find(params[:class_id]).questions.recent.top_level.page(params[:page]).per(5)
  ...
  def view_question
   ...
   #   end
   # end
    @questions = @class.questions.where(:flagged_as => 'question').recent.page(params[:page]).per(5)
    respond_to do |format|
      format.js #{ redirect_to "/classes/#{@class.slug}/" }
      format.html 
    end
    #  if request.xhr?
    #    render :partial => 'questions/list_item', :locals => {:questions => @questions.page(params[:page]).per(5), :class_id => params[:offering_id], :question_type => 'class'}
    #  end
  end
  ...
  def view_comment
   ...
   #   end
   # end
    @questions = @class.questions.where(:flagged_as => 'posting').recent.page(params[:page]).per(5)
    respond_to do |format|
      format.js #{ redirect_to "/classes/#{@class.slug}/" }
      format.html 
    end
  ...
  def view_all
   ...
   #     @questions << question
   # end
   # @questions.page(params[:page]).per(5) 
    @questions = @class.questions.recent.page(params[:page]).per(5) 
     respond_to do |format|
       format.js #{ redirect_to "/classes/#{@class.slug}/" }
       format.html 
     end      
  ...
  def find_questions
    ...
    if params[:filter]
      ...
      @questions = @class.questions.recent.top_level.page(params[:page]).per(5)
    end
    @questions
  end

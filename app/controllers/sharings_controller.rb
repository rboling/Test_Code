class SharingsController < ApplicationController
  ...
  def new
    @sharing = Permission.new
    #@sharing = Sharing.new
    @position = params[:position] ||= "below-right"
    @object_id = params[:object_id] #this is the id of the object (i.e. note_382)
    @object_to_share = params[:object_to_share] #this is the type of the object (ie Notes)
  end

  def create
    debugger
    @sharing = Permission.new(params[:sharing])
    ...
    #generate_post(@sharing) #this is to generate a post with a note attached
  end

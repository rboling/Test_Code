  ...
  def create
   ...
   # Rails.logger.info(omniauth["credentials"])
    if authentication && authentication.user.present?         # Authentication already exists, Sign In & Redirect
      ...
      #the following three lines were not originally there
      @user = authentication.user
      @user.omniauth_facebook_token = omniauth.credentials.token
      @user.save
      ...
      current_user.save
                                        # User is logged in, add authentication & proceed

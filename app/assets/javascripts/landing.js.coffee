class landingPage
  ...
  setup: -> 
    ...
    #Sign-up Form 
    @signup1 = $('.signup')
    @form = $('#new_user', @signup1)
    @form_submit = $("input[type='submit']", @form)
    @form_prefix = $('.url-prefix', @form)
    @info_message = $('#info_message', @signup1)
    @success_message = $('#success_message', @signup1)
...
$ -> 
  ...
  if $('body').hasClass('home-landing_page')
   # alert($(".browser-quad").css "bottom") 
   # alert($(".browser-main").css "bottom")
    if $.browser.msie
      $(".browser-quad").css "bottom", "-90px"
      $(".browser-main").css "bottom", "-65px"
    landing.setup()
    if $.browser.msie
      $(".browser-quad").css "bottom", "-90px"
      $(".browser-main").css "bottom", "-65px"
    ...
    $(".email-container.a").twipsy({ #addition of .a  essentially disables this twipsy alert - need to fix bc doesn't work after alert message
      placement: 'above'
      offset: 0
      title: ->
    ...
    if $.browser.msie
      $(".browser-quad").css "bottom", "-90px"
      $(".browser-main").css "bottom", "-65px"
   # $foo = $(".browser-quad").removeClass 'drop'
   # alert($(".browser-quad drop").css "bottom") 
   # alert($(".browser-main drop").css "bottom")
   # $(".browser-main").removeClass 'drop'
   # $foo.css "bottom", "-90px"
   # alert($foo.css "bottom") 
   # alert($(".browser-main").css "bottom")   
   # $(".browser-quad").css "bottom", "-90px"
   # $(".browser-main").css "bottom", "-65px"

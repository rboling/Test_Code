class Note < ActiveRecord::Base
  ...
  attr_accessible :permission_settings, :permission_setting_attributes, :permission_settings_attributes, :content, :name, :folder_id, :notebook_id, :upload, :doc, :doc_type, :doc_preserved

  has_one :permission_setting, :dependent => :destroy
  has_many :permissions, :through => :permission_setting, :dependent => :destroy
  has_and_belongs_to_many :users
  has_and_belongs_to_many :groups
  has_many :offering_permissions, :dependent => :destroy
  belongs_to :notebook
  belongs_to :folder
  accepts_nested_attributes_for :permission_setting, allow_destroy:true
  validates_presence_of :user_id
  
  scope :unsorted, lambda { where("notebook_id = ? AND folder_id = ?", nil, nil) }
